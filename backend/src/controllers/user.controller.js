...
const deleteHistoryItem = async (req, res) => {
    ...
    try {
        ...
        const deleted = await Meeting.findOneAndDelete({ _id: id, user_id: user.username });
        if (!deleted) {
            return res.status(httpStatus.NOT_FOUND).json({ message: "Meeting not found" });
        }

        return res.status(httpStatus.OK).json({ message: "Meeting removed from history" });
    } catch (e) {
...
export { login, register, getUserHistory, addToHistory, deleteHistoryItem }